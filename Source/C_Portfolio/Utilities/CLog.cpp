...
{
	UE_LOG(GameProject/*Category name*/, Display, L"%d", InValue);//매크로
}

void CLog::Log(float InValue)
{
	UE_LOG(GameProject, Display, L"%f", InValue);//매크로
}

void CLog::Log(const FString & InValue)
{
	UE_LOG(GameProject, Display, L"%s", *InValue);//매크로
}

void CLog::Log(const FVector & InValue)
{
	UE_LOG(GameProject, Display, L"%s", *InValue.ToString());//매크로
}
...
void CLog::Log(const UObject * InObject)
{
	...
	UE_LOG(GameProject, Display, L"%s", *str);//매크로
}
...
void CLog::LogError(const FString & InFuncName, int32 InLineNumber, FString InMessage)
{
	...
	UE_LOG(GameProject, Error, L"%s", *str);//매크로
